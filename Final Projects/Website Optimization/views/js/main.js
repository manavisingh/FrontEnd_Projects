...
//Changed the "querySelectorALl" to getElementById" because the latter is less expensive and can be used for simple js queries.
  ...
  //removed the variables out of the for loop so that the DOM doesnt have to be modified each time the loop runs.
  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    // given a variable the value of frequently repeated DOM selector: document.getElementsByClassName("randomPizzaContainer");
...
//Changed the i max value from 100 to 20 to keep in mind the number of pizzas that would actually render on screen once the page loads.
for (var i = 2; i < 20; i++) {
...
function updatePositions() {
  ...
  //Moved all calculations from within the loop to outside of it and saved the values in variables which are used in the for loop.
  var items = document.querySelectorAll('.mover');
  var scrollTop = document.body.scrollTop;
  
  //Saved calculations inside the for loop
  var pizza_array = [];
  var i;
  
  for (var i = 0; i < 5; i++) {
	pizza_array.push(Math.sin((scrollTop / 1250) + (i % 5)));
	}
	
  for (var i = 0; i < items.length; i++) {
    var phase = pizza_array[i % 5];
